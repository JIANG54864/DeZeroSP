...
- stride (int 或 (int, int)): 滤波器应用的步幅。
- pad (int 或 (int, int)): 输入数组的空间填充宽度。
- nobias (bool): 如果为 True，则该函数不使用偏置项。
- in_channels (int 或 None): 输入数组的通道数。如果为 None，参数初始化将延迟到第一次前向传播时确定输入大小。
        """
        super().__init__()
...
class RNN(Layer):
    def __init__(self, hidden_size, in_size=None):
        """一个使用 tanh 激活函数的 Elman 循环神经网络（RNN）。

参数：
- hidden_size (int): 隐藏状态的特征数量。
- in_size (int): 输入的特征数量。如果未指定或为 `None`，参数初始化将延迟到第一次调用 `__call__(x)` 时确定输入大小。
